...
<Check>
You should see the Task Dojo landing page! 🥋
</Check>
...
│   ├── Disciples.tsx    # Sensei features
│   ├── ui/              # shadcn/ui components
│   └── ...
├── utils/               # Utility functions
│   ├── api.ts           # API client
│   ├── belt-colors.ts   # Belt calculations
...
Save file
Function reloads automatically ✨
```
...
async function apiCall(endpoint: string, options: RequestInit) {
  ...
  if (DEBUG) {
    console.log(`${response.ok ? '🟢' : '🔴'} API Response:`, await response.clone().json())
  }
...
<Check>
You're ready to develop! Make changes, watch them update instantly, and build something amazing. 🚀
</Check>
```
